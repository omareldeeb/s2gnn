...
if __name__ == '__main__':
    ...
    save_dir = "./datasets/Coll/processed_coll_small"
    os.makedirs(save_dir, exist_ok=True)

    print("Reading 10 train samples...")
    train_structs = read(os.path.join(base_path, "coll_v1.2_AE_train.xyz"), index="0:10")
    train_energies = np.array([atoms.get_potential_energy() for atoms in train_structs])
    mean_energy = np.mean(train_energies)
    print(f"Train mean energy (10 samples): {mean_energy:.6f} eV")

    R, Z, E, F, N = process_structures(train_structs, mean_energy)
    save_npz(os.path.join(save_dir, "coll_train.npz"), R, Z, E, F, N)

    print("Reading 10 val samples...")
    val_structs = read(os.path.join(base_path, "coll_v1.2_AE_val.xyz"), index="0:10")
    R, Z, E, F, N = process_structures(val_structs, mean_energy)
    save_npz(os.path.join(save_dir, "coll_val.npz"), R, Z, E, F, N)

    print("Reading 10 test samples...")
    test_structs = read(os.path.join(base_path, "coll_v1.2_AE_test.xyz"), index="0:10")
